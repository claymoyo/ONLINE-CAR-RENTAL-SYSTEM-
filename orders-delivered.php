...
?>
<title>Orders Delivered | Novaride #1 Car Rental Company</title>
<style media="screen">
  .main-nav {
    display: flex;
    gap: 10px;
  ...
  <!-- Page Feets Single Start -->
	<div class="page-fleets-single" style="padding-top:0 !important;">
      ...
      <div class="page-body" style="min-height:500px;">
        <h3 style="margin-top:30px;">Delivered to Customer</h3>
        <hr>
        <?php
          // Check for any record of booking
          $fetch_orders_received_stmt = $pdo->query("SELECT * FROM orders WHERE order_status_id = 3 ORDER BY id DESC ");
          ...
          if( $fetch_orders_received_stmt->rowCount() < 1 )
          {
            ...
            <div class="jumbotron-large" style="text-align:center;padding:90px;">
              ...
              <h5 style="margin-top:20px;">You don't have any delivered orders yet!</h5>
              <p>You have no delivered orders at this time. Manage your new bookings and dispatched orders</p>
            <?php
          }
          else
          {
                    ...
                    while($orders_received = $fetch_orders_received_stmt->fetch(PDO::FETCH_ASSOC))
                    {
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <button type="button" class="btn btn-sm btn-short btn-returned" id="<?= $order_id; ?>" style="padding: 5px !important;">Mark as Returned</button>
                        </td>
                      </tr>
                      <?php

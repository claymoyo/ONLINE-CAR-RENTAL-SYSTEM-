...
?>
<title>Orders Received | Novaride #1 Car Rental Company</title>
<style media="screen">
  .main-nav {
    display: flex;
    gap: 10px;
  ...
  <!-- Page Feets Single Start -->
	<div class="page-fleets-single" style="padding-top:0 !important;">
      ...
      <div class="page-body" style="min-height:500px;">
        <h3 style="margin-top:30px;">Orders Received</h3>
        <hr>
        <?php
          // Check for any record of booking
          $fetch_orders_received_stmt = $pdo->query("SELECT * FROM orders WHERE order_status_id = 1 ORDER BY id DESC ");
          ...
          if( $fetch_orders_received_stmt->rowCount() < 1 )
          {
            ...
            <div class="jumbotron-large" style="text-align:center;padding:90px;">
              ...
              <h5 style="margin-top:20px;">You don't have any new bookings yet!</h5>
              <p>You're all caught up on all new bookings. Proceed to manage your active bookings</p>
            <?php
          }
          else
          {
                    ...
                    while($orders_received = $fetch_orders_received_stmt->fetch(PDO::FETCH_ASSOC))
                    {
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <button type="button" class="btn btn-sm btn-short btn-dispatched" id="<?= $order_id; ?>" style="padding: 5px !important;">Mark as Dispatched</button>
                        </td>
                      </tr>
                      <?php
